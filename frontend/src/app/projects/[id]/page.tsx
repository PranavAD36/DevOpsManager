        ...
        {project && (
          <>
            ...
            <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
              <div className="space-y-8">
                      ...
                      {repositories.map((repository) => (
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-sm text-slate-400">
                                {repository.github_description ||
                                  "No description provided."}
                              </p>
                            </div>
                            <button
                              className="rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-200 hover:border-cyan-400 disabled:opacity-60"
                              disabled={busyRepository === repository.id}
                              onClick={() =>
                                void refreshRepository(repository.id)
                              }
                            >
                          ...
                          <div className="mt-4 grid gap-2 text-xs text-slate-400 sm:grid-cols-4">
                            ...
                            <span>
                              {repository.open_issues_count ?? 0} issues
                            </span>
                          </div>
                ...
                <section>
                  ...
                  {analysisRuns.length === 0 ? (
                    <p className="mt-3 text-slate-400">No analysis runs yet.</p>
                  ) : (
                    <div className="mt-3 space-y-3">
                ...
                <section>
                  ...
                  {filteredIssues.length === 0 ? (
                    <p className="mt-3 text-slate-400">No matching issues.</p>
                  ) : (
                    <div className="mt-3 space-y-3">
              ...
              <section className="h-fit rounded-xl border border-slate-800 bg-slate-900/70 p-6">
                <h2 className="text-lg font-semibold text-white">
                  Connect GitHub
                </h2>
                <p className="mt-2 text-sm text-slate-400">
                  Authorize GitHub and choose a repository for this project.
                </p>
                <Link
                  className="mt-5 block w-full rounded-lg bg-cyan-400 px-4 py-3 text-center font-semibold text-slate-950"
                  href="/github/connect"
                >
                  Connect GitHub
                </Link>
                <div className="mt-8 border-t border-slate-800 pt-6">
                  <h3 className="font-medium text-white">
                    Legacy URL connection
                  </h3>
                  <form className="mt-4 space-y-4" onSubmit={connectRepository}>
                    ...
                    <button
                      className="w-full rounded-lg border border-slate-700 px-4 py-3 font-semibold text-slate-200 disabled:opacity-60"
                      disabled={connecting}
                      type="submit"
                    >
